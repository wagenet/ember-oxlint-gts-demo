...
A minimal modern Ember app — Vite, `.gts` components, TypeScript — set up so that
**oxlint's type-aware rules report inside `<template>`**, anchored at the right offsets
...
| **tsgolint** | honour the tsconfig's `contentMappers`, which is how TypeScript 7 hands `.gts` to Glint | [tsgolint#1166](https://github.com/oxc-project/tsgolint/pull/1166) |
...
oxlint. Set `OXLINT_INTERNAL_DIR` to reuse a build staged elsewhere.

Requirements: Node >= 22.21.1 (`ember-content-mapper` spawns a bare `node`), Go and Rust
toolchains for the two builds. Verified on darwin-arm64.

## How a `.gts` file gets linted

One config override drives **three** passes. This is the part worth understanding:
...
1. **JS-plugin pass.** `ember-eslint-parser` genuinely parses the file, and bridged
   ESLint-plugin rules run on that AST. `ember/template-no-let-reference` below is one.
2. **Native pass.** oxlint's own Rust rules run against a *shadow source* derived from
   that parse, which is why template usage counts — see `greeting.gts`.
3. **Type-aware pass.** The override's *presence* is what makes an extension oxlint
   doesn't recognise eligible for tsgolint. The parsed AST is never handed over: tsgolint
   receives the **original path** and re-derives the content itself through
   `contentMappers`, which runs `ember-content-mapper` (wrapping Glint) to produce
   TypeScript for typescript-go to check.

So a `.gts` is parsed twice by two different things — `ember-eslint-parser` for rules that
run in-process, `ember-content-mapper` for the type-aware ones.
...
a checked-in config must not be able to grant a repo the right to run its own code. Hence
`--run-external-code`, which a `languageOptions.parser` override deliberately does not imply.

## What it reports

`pnpm lint:oxlint` is **expected to fail**: the findings are the demo. Each is marked
...
| `app/components/banner.gts:7` | `ember(template-no-let-reference)` on `{{message}}` | JS plugin |
...
sections and never touches `<template>`:
...
A `.gts` diagnostic arriving with no fix attached is expected, not a bug. (`--fix` leaves the
now-redundant parens; `pnpm format` cleans them up.)

## Telling a mapper problem from a rule problem

Drop the `.gts` override and the file stops being eligible at all, so you get silence rather
than a misleading diagnostic:
...
natively parseable file from that tsconfig. Programs are built per requested file, so linting
a single `.gts` — which is every editor request — gets the bare
`unsupported-file-extension` and no reason. Worth knowing before you go looking for the cause.
...
**`tsconfig.json`** — the entry that makes `.gts` type-checkable at all:
...
**`package.json`** — `ember-content-mapper`, `ember-eslint-parser` (oxlint resolves
`languageOptions.parser` as a bare specifier from the package being linted, so it is declared
directly rather than leaned on transitively), `@glint/ember-tsc` pinned to the `~1.11` line
the mapper wants, and TypeScript twice:
...
`lint:types` runs the TypeScript 7 copy as `tsc --noEmit --runExternalCode`. oxlint itself
needs neither: tsgolint has typescript-go compiled in.

`.gts` and `.gjs` imports need **explicit extensions** (`./counter.gts`, not
`ember-oxlint-gts-demo/components/counter`) — the `paths` alias resolves `.ts` but not `.gts`,
and you get `TS2307 Cannot find module` if you forget.
...
`prefer-const` and a `warp-drive` rule that this oxlint config does not cover — `correctness`
is the only category enabled, and `eslint-plugin-warp-drive` is not in `jsPlugins`. Worth
seeing rather than hiding.

The generated GitHub Actions workflow was removed: `pnpm lint` cannot run anywhere the two
binaries have not been built by hand.
...
so type-aware `.gts` in an editor needs a build that defaults it on, which is a local patch,
not something on the PR branches.
...
- **Template-shaped rules.** oxlint has no native `ember` plugin and no template linting.
  `eslint-plugin-ember` rules run bridged, and rules needing `getScope()`-style resolution on
  the Glimmer AST remain blocked — see
...
- **Bridged rules using code-path analysis.** oxlint's JS-plugin bridge rejects
  ...
  here and turned back off in the `.gts` override to show the shape of the workaround; leave
  it on and every `.gts` gets this instead of a report:
  ...
  It takes down the whole JS-plugin pass for that file. The type-aware pass is unaffected —
  a separate path — so `no-unnecessary-condition` still reports while the ember rules go
  missing, which makes this easy to misread as a rule that simply found nothing.
- **Bridged rules needing type information.** The bridge supplies no parser services, and
  ...
  work.
- **Diagnostics whose fix is unexpressible in the template.** `typescript/dot-notation` is the
  clearest case: a named block `<:actions>` becomes `blocks["actions"]` in the mapped
  TypeScript, and `.actions` is not something you can write in a template. Not enabled here.
- **Linux/CI.** Only built and run on darwin-arm64.
...
- [`NullVoxPopuli/ember-content-mapper`](https://github.com/NullVoxPopuli/ember-content-mapper)
  ...
  type-checking with linting deliberately stripped. This repo is that setup plus the linting.
